  ...
  while (rclcpp::ok())
  {
    switch (state_)
    {
      ...
      case PLANNING: {
        ...
        {
          RCLCPP_WARN(get_logger(), "mode %d, planning, planner timeout %f, reset status",
      ...
      case TRACKINGROTATERECOVERY: {
        ...
        while (!has_new_msg && i++ < 12 && rclcpp::ok())
        {
          ...
          {
            has_new_msg = true;
            RCLCPP_WARN(get_logger(), "TRACKINGROTATERECOVERY have new msg %ld",
                        goals_queue_.size());
            publishZeroVelocity();
            last_valid_plan_time_ = now();
            state_ = NavState::PLANNING;
        ...
        if (!has_new_msg)
        {
          ...
          if (getRobotPose(target))
          {
            ...
            if (goals_queue_.empty())
            {
              ...
              {
                RCLCPP_INFO(get_logger(), "After Rotate Recovery-2, find new target, %ld",
                            goals_queue_.size());
                last_valid_plan_time_ = now();
                state_ = PLANNING;
              }
            ...
            {
              RCLCPP_INFO(get_logger(), "After Rotate Recovery-1, find new target, %ld",
                          goals_queue_.size());
              last_valid_plan_time_ = now();
              state_ = PLANNING;
            }
        ...
        {
          RCLCPP_FATAL(get_logger(), " has_new msg ? %ld, continue and replan",
                       goals_queue_.size());
        }
        // 3. return ready
      }
...
nav2_util::CallbackReturn MoveBase::on_configure(const rclcpp_lifecycle::State& state)
{
  ...
  RCLCPP_INFO(get_logger(), "Controller frequency set to %.4fHz, period %d", controller_frequency_,
              period_);
  ...
  for (size_t i = 0; i != controller_ids_.size(); i++)
  {
    try
    {
      ...
      controller->configure(node, controller_ids_[i], controller_costmap_ros_->getTfBuffer(),
...
void MoveBase::computeControl()
{
  ...
  try
  {
    ...
    if (need_update_goal_checker)
    {
      ...
      nav_2d_utils::transformPose(tf_, controller_costmap_ros_->getGlobalFrameID(), end_pose,
                                  end_pose, tolerance);
      goal_checker_->reset();

      RCLCPP_INFO(get_logger(), "Path end point is (%.2f, %.2f)", end_pose.pose.position.x,
    ...
    RCLCPP_INFO(get_logger(), "Publishing velocity at time %.2f [%f,%f], timecost %f",
                now().seconds(), cmd_vel_2d.twist.linear.x, cmd_vel_2d.twist.angular.z,
                time_span_in_ms);

    if (time_span_in_ms >= period_)
    {
      RCLCPP_WARN(get_logger(), "Control loop missed its desired rate of %.4fHz, time_cost %f",
...
bool MoveBase::transformPose(const std::string& target_frame,
...
void MoveBase::trackingPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  RCLCPP_INFO(get_logger(), "tracking_pose in %s, [%f, %f]", msg->header.frame_id.c_str(),
