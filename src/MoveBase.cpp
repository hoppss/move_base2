  ...
  default_controller_types_{"teb_local_planner::TebLocalPlannerROS"}
{
  ...
  controller_costmap_ros_ =
    ...
    "local_costmap",
    std::string{get_namespace()}, "local_costmap");
      ...
      case PLANNING: {
          ...
          if (navi_mode_ == NavMode::NavMode_Track && time_used > 10.0) {
            ...
            if (trapped_recovery_->isTrapped()) {
              ...
              reporter_->report(
                static_cast<int>(navi_mode_), automation_msgs::msg::NavStatus::FAILED_NOPATH,
      ...
      case TRACKINGROTATERECOVERY: {
          ...
          if (!has_new_msg) {
            ...
            if (p_in_camera.pose.position.y > 0) {
              ...
              reporter_->report(
                static_cast<int>(navi_mode_), automation_msgs::msg::NavStatus::FAILED_NOTRACKGOAL,
            ...
            if (getRobotPose(target)) {
              ...
              if (goals_queue_.empty()) {
                RCLCPP_INFO(
                  get_logger(), "After Rotate Recovery-1, no new target, return origin pose");
                i = 0;
                ...
                if (goals_queue_.empty()) {
                  RCLCPP_INFO(
                    get_logger(), "After Rotate Recovery-2, no new target, reset State to READY");
                  state_ = NavState::READY;
                  resetState();
                } else {
                  RCLCPP_INFO(
...
nav2_util::CallbackReturn MoveBase::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  for (size_t i = 0; i != controller_ids_.size(); i++) {
    try {
      ...
      controller->configure(
        ...
        controller_costmap_ros_->getTfBuffer(), controller_costmap_ros_);
  ...
  service_handle_ = this->create_service<automation_msgs::srv::NavigateToPose>(
    "NaviTo", std::bind(
      &MoveBase::handleService, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  tracking_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
    "tracking_pose", sub_qos, std::bind(
      &MoveBase::trackingPoseCallback, this,
      std::placeholders::_1));
  ...
  std::shared_ptr<automation_msgs::srv::NavigateToPose::Response> response)
{
  ...
  RCLCPP_INFO(
    get_logger(),
    "NaviTo: planner [%s] controller [%s], pose [%f, %f, %f], is_cancel [%d], queue_size [%d]",
    request->planner_id.c_str(), request->controller_id.c_str(), request->goal.pose.position.x,
    request->goal.pose.position.y, tf2::getYaw(
      request->goal.pose.orientation), static_cast<int>(request->is_cancel),
    goals_queue_.size());
...
void MoveBase::computeControl()
{
  ...
  try {
    ...
    if (need_update_goal_checker) {
      ...
      rclcpp::Duration tolerance(rclcpp::Duration::from_seconds(
          controller_costmap_ros_->getTransformTolerance()));
      nav_2d_utils::transformPose(
        tf_,
        controller_costmap_ros_->getGlobalFrameID(), end_pose, end_pose, tolerance);
      goal_checker_->reset();
    ...
    if (isGoalReached()) {
      ...
      if (navi_mode_ == NavMode::NavMode_Track) {
        ...
        state_ = NavState::READY;
        reporter_->report(
          static_cast<int>(navi_mode_), automation_msgs::msg::NavStatus::SUCCESS_REACHED,
          "reach_to_goal!");
      }
    ...
    RCLCPP_INFO(
      get_logger(), "Publishing velocity at time %.2f [%f,%f], timecost %f", now().seconds(),
...
void MoveBase::planThread()
{
  ...
  while (rclcpp::ok()) {
    ...
    if (path.poses.size() == 0) {
      ...
      RCLCPP_INFO(
        get_logger(), "planner_thread: Found valid path of size %lu to (%.2f, %.2f), publish",
  ...
  std::shared_ptr<automation_msgs::srv::NavMode::Response> res)
{
  ...
  switch (req->sub_mode) {
    ...
    case automation_msgs::srv::NavMode::Request::TRACK_S: {
        RCLCPP_INFO(
          this->get_logger(), "Change mode to tracking, set current_controller_ to FollowPath");
        if (navi_mode_ != NavMode::NavMode_Track) {
  ...
  geometry_msgs::msg::PoseStamped & out_pose)
{
  ...
  try {
    ...
    RCLCPP_ERROR(
      get_logger(), "transformPose: No Transform available Error looking up robot pose: %s\n",
      ex.what());
...
void MoveBase::trackingPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  if (navi_mode_ != NavMode::NavMode_Track) {
    RCLCPP_WARN_THROTTLE(
      get_logger(),
      *get_clock(), 10000, "tracking callback, current is nott TrackingMode");
    return;
  }
  ...
  RCLCPP_INFO(
    get_logger(), "tracking_pose in %s, [%f, %f]",
