                                 ...
                                 std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // vel_pub_ = vel_pub;
  RCLCPP_INFO(logger_, "global_frame %s, local_frame %s", global_frame_.c_str(),
              base_frame_.c_str());
  ...
  {
    RCLCPP_INFO(logger_, "i %d, [%f, %f, %f]", i, footprint_[i].x, footprint_[i].y,
                footprint_[i].z);
  }
  // initFootprint();
...
bool TrappedRecovery::isTrapped()
{
  ...
  transformFootprint(global_pose.pose.position.x, global_pose.pose.position.y, yaw, footprint_,
                     oriented_footprint);
...
double TrappedRecovery::scoreFootprint(std::vector<geometry_msgs::msg::Point> oriented_footprint)
{
  ...
  for (size_t i = 0; i < oriented_footprint.size() - 1; ++i)
  {
    ...
    if (!controller_costmap_->getCostmap()->worldToMap(oriented_footprint[i].x,
    ...
    if (!controller_costmap_->getCostmap()->worldToMap(oriented_footprint[i + 1].x,
  ...
  if (!controller_costmap_->getCostmap()->worldToMap(oriented_footprint.back().x,
  ...
  // get the cell coord of the first point
  if (!controller_costmap_->getCostmap()->worldToMap(oriented_footprint.front().x,
...
bool TrappedRecovery::transformPose(const std::string frame,
