...
public:
  ...
  {
    last_body_info_ = *msg;
    RCLCPP_INFO(this->get_logger(), "cnt %d, size %d", (int)last_body_info_.count, (int)last_body_info_.infos.size());
  }
  ...
  bool call(interaction_msgs::msg::Body& bbox)
  {
    ...
    char text[100];
    sprintf(text, "reid-bbox=%d,%d,%d,%d", bbox.roi.x_offset, bbox.roi.y_offset, bbox.roi.width, bbox.roi.height);
    ...
    {
      RCLCPP_ERROR(node->get_logger(), " %s call failed, result %d, msg %s", service_name.c_str(),
                   static_cast<int>(result_future.get()->result), result_future.get()->msg);
      return false;
    }
    else
    {
      RCLCPP_INFO(node->get_logger(), "%s call success, result %d, msg %d", service_name.c_str(),
                  static_cast<int>(result_future.get()->result), result_future.get()->msg);
    }
    return true;
  }
